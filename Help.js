...
function getHelpArticleDefaults_() {
  return {
    ...
    'accounting-operating-rules': {
      title: 'Accounting Operating Rules',
      content: [
        '# Accounting Operating Rules',
        '',
        '**Status:** Approved and locked on 28-JUL-2026.',
        '',
        '## Storage responsibilities',
        '- Google Apps Script runs Nilavaram.',
        '- Firestore is the authoritative live accounting database.',
        '- `E:\\nn\\Nilavaram Data` stores local evidence, exports and backups; it is not the live database.',
        '- `E:\\nn\\Nilavaram Data\\04 OneDrive Sync` is the approved source for the owner’s file-sync application.',
        '- OneDrive stores cloud copies of evidence. Uploading a file does not itself create an accounting transaction.',
        '',
        '## Transaction identity',
        '- Every transaction receives an immutable internal ID and a readable transaction number.',
        '- The transaction ID is never reused.',
        '- Every record identifies its schema version, source, date, description, currency, entity, amount, status, creator and timestamps.',
        '- Posting, reconciliation and document status remain separate.',
        '',
        '## Journal rules',
        '- Every transaction has separate journal-line records.',
        '- Each line belongs to one transaction and has a stable line ID and line number.',
        '- Money is stored as integer cents to prevent rounding errors.',
        '- Each line is either a debit or a credit, never both.',
        '- Total debits must equal total credits before posting.',
        '- Only active entry accounts from the Chart of Accounts may be posted.',
        '- The transaction header and all lines are saved atomically: either all succeed or none are saved.',
        '',
        '## Posting and reconciliation',
        '- A valid balanced transaction posts immediately.',
        '- Reconciliation does not delay posting.',
        '- Posted but unreconciled transactions remain in reports and will show a red unresolved alert.',
        '- Reconciliation marks the existing transaction; it does not create another accounting event.',
        '- Missing evidence is disclosed separately and does not silently remove a valid posted transaction.',
        '',
        '## Corrections and audit',
        '- Posted transactions are never silently overwritten or deleted.',
        '- Corrections preserve old values, new values, reason, user and time.',
        '- Material accounting corrections will use reversing and replacement journal lines.',
        '- Locked or closed-period transactions require controlled reopening.',
        '- Audit history is append-only through the Nilavaram workflow.',
        '',
        '## Documents and duplicates',
        '- Firestore stores document metadata and links, not large file contents.',
        '- Moving a local evidence file does not change the journal.',
        '- Transaction IDs, import idempotency keys and file hashes prevent duplicate posting and uploads.',
        '- Same-date and same-amount warnings require review; they are not automatic proof of duplication.',
        '',
        '## Safe user workflow',
        ...
        '7. Correct through the controlled workflow; never replace Firestore records or delete files manually.',
        '',
        '## Implementation status',
        '- Step 1 — transaction header and journal-line structure: completed.',
        '- Step 2 — immediate balanced posting and duplicate-post prevention: completed.',
    ...
    'admin-technical-guide': {
      ...
      content: [
        ...
        '- VS Code folder name: `Nilavaram`.',
        '- This is the authoritative program-code working copy, not a document archive.',
        '',
        '## Development flow',
        '```',
        ...
        '- Production web app: `https://script.google.com/macros/s/AKfycbxMMqgLL6xmJp__dI4vDHj0zZ_6ZyZsb_-KsspdNU99WXW1ZJrRzJTaVObTJ8C2s-3Q/exec`.',
        ...
        '- Local folder: `C:\\Users\\theso\\Documents\\nn\\Nilavaram`.',
        '- Primary branch: `main`.',
        ...
        '',
        '## External HDD and OneDrive operating note',
        '- External HDD: `E:\\` (`My Passport`), approximately 4.66 TB total and 3.78 TB free when checked on 2026-07-28.',
        '- External code backup: `E:\\nn\\Nilavaram`. It is a copy and must not replace the authoritative VS Code/Git working copy on C:.',
        '- External data root: `E:\\nn\\Nilavaram Data`.',
        '- New/unreviewed files: `E:\\nn\\Nilavaram Data\\00 Inbox`.',
        '- Source documents: `E:\\nn\\Nilavaram Data\\01 Source Documents`.',
        '- Local exports: `E:\\nn\\Nilavaram Data\\02 Exports`.',
        '- Firestore export/backup files: `E:\\nn\\Nilavaram Data\\03 Firestore Backups`.',
        '- File-sync source for OneDrive: `E:\\nn\\Nilavaram Data\\04 OneDrive Sync`.',
        '- Long-term external archive: `E:\\nn\\Nilavaram Data\\99 Archive`.',
        '- Primary OneDrive repository: `vmurugan@hotmail.com` with approximately 999.8 GB available when checked on 2026-07-28.',
        '- Local OneDrive view: `C:\\Users\\theso\\OneDrive` with Files On-Demand.',
        '- Keep the external HDD copy as the local source or backup. Upload a copy manually through OneDrive on the web, or configure the selected file-sync application to copy the OneDrive Sync folder.',
        '- Do not move the only copy into OneDrive. Verify the cloud upload before deleting or reorganizing any external-HDD source.',
        '- Do not select Always keep on this device for the complete archive; this would consume laptop space.',
        '',
        '## Credential register',

...
const OUT = "SVT_Nexus_Financial_Model_V2.xlsx";
...
const title = (s, text, endCol="H") => {
  ...
  s.mergeCells(`A2:${endCol}2`); s.getRange("A2").values=[["Version 2 | Model date: 21 Aug 2026 | Prepared for management and securities counsel review"]];
...
// Assumptions
const a=wb.worksheets.add("Assumptions"); title(a,"SVT Nexus — Version 2 Assumptions","H");
...
const fin=[
 ...
 ["Annual coupon rate",0.09,"Definitive working term","Accrues from actual receipt date; ACT/365"],
 ["Cash interest frequency (months)",3,"Definitive working term","Quarterly payments; expense accrues monthly"],
 ["Conversion timing","After 2029","Management clarification","No conversion forecast during 2026-2029"],
 ...
 ["Liquidity reserve target",0.175,"Management framework","Midpoint of disclosed 15%-20% range"],
...
const opex=[["CEO compensation",120000,"Management assumption"],["Other personnel / contractors",300000,"Management assumption"],["Technology / AI / cloud",60000,"Planning estimate"],["Sales & marketing",120000,"Planning estimate"],["Legal / accounting / audit / tax / compliance",80000,"Planning estimate"],["Insurance",30000,"Planning estimate"],["Travel",40000,"Planning estimate"],["Office / administrative",30000,"Planning estimate"],["Other consultants / contractors",40000,"Planning estimate"]];
...
for(let i=0;i<months.length;i++){
 ...
 const rampCol=col(m+1); const churnFactor=y===2027?"1":y===2028?"(1-'Assumptions'!$B$11)":"(1-'Assumptions'!$B$11)^2";
 const priceGrowth=y===2027?"1":y===2028?"(1+'Assumptions'!$B$12)":"(1+'Assumptions'!$B$12)^2";
 rev.getRange(`${c}4`).formulas=[[`=MIN(6,'Client Ramp'!${rampCol}$9)*${churnFactor}`]];
 rev.getRange(`${c}5`).formulas=[[`=MIN(3,MAX(0,'Client Ramp'!${rampCol}$9-6))*${churnFactor}`]];
 rev.getRange(`${c}6`).formulas=[[`=MIN(3,MAX(0,'Client Ramp'!${rampCol}$9-9))*${churnFactor}`]];
...
const tranches=[["Tranche 1",new Date(Date.UTC(2026,11,1)),150000,"Management schedule"],["Tranche 2",new Date(Date.UTC(2027,0,1)),300000,"Management schedule"],["Tranche 3",new Date(Date.UTC(2027,1,1)),300000,"Management schedule"]];
for(let k=0;k<9;k++) tranches.push([`Tranche ${k+4}`,new Date(Date.UTC(2027,k+2,1)),250000,"Management schedule"]);
...
note.getRange("A18:A22").values=[["Interest expense accrued"],["Cash interest paid (quarterly)"],["Ending accrued interest payable"],["Outstanding principal"],["Conversion / principal repayment"]];
for(let i=0;i<months.length;i++){
 ...
 note.getRange(`${c}21`).formulas=[[`=SUMIF($B$5:$B$16,"<="&EOMONTH(${c}$4,0),$C$5:$C$16)`]];
 note.getRange(`${c}22`).values=[[0]];
}
section(note,"A18:AO18"); section(note,"A21:AO22"); note.getRange("E5:AO22").format.numberFormat=moneyFmt; note.getRange("A:A").format.columnWidth=28; note.getRange("B:B").format.columnWidth=15; note.getRange("C:C").format.columnWidth=15; note.getRange("D:D").format.columnWidth=22; note.getRange("E:AO").format.columnWidth=11; note.freezePanes.freezeRows(4); note.freezePanes.freezeColumns(4);
...
for(let i=0;i<months.length;i++){ const c=monthCol(i), nc=col(i+5); inc.getRange(`${c}4`).formulas=[[`='Revenue Model'!${c}12`]]; inc.getRange(`${c}5`).formulas=[[`='Revenue Model'!${c}13`]]; inc.getRange(`${c}6`).formulas=[[`=SUM(${c}4:${c}5)`]]; inc.getRange(`${c}7`).formulas=[[`='Operating Model'!${c}13`]]; inc.getRange(`${c}8`).formulas=[[`=SUM(${c}6:${c}7)`]]; inc.getRange(`${c}9`).formulas=[[`=-'Convertible Note'!${nc}18`]]; inc.getRange(`${c}10`).formulas=[[`=SUM(${c}8:${c}9)`]]; inc.getRange(`${c}11`).formulas=[[`=-MAX(0,${c}10*'Assumptions'!$B$9)`]]; inc.getRange(`${c}12`).formulas=[[`=SUM(${c}10:${c}11)`]]; }
...
for(let i=0;i<months.length;i++){
 ...
 cf.getRange(`${c}11`).formulas=[[`=SUMIF('Convertible Note'!$B$5:$B$16,${c}$3,'Convertible Note'!$C$5:$C$16)`]]; cf.getRange(`${c}12`).formulas=[[`='Convertible Note'!${nc}19`]]; cf.getRange(`${c}13`).formulas=[[`=SUM(${c}11:${c}12)`]]; cf.getRange(`${c}14`).formulas=[[`=SUM(${c}10,${c}13)`]]; cf.getRange(`${c}15`).formulas=[[`=SUM(${c}4,${c}14)`]]; cf.getRange(`${c}16`).formulas=[[`='Convertible Note'!${nc}21*'Assumptions'!$B$15`]]; cf.getRange(`${c}17`).formulas=[[`=${c}15-${c}16`]]; cf.getRange(`${c}18`).formulas=[[`='Convertible Note'!${nc}21`]];
}
...
const uses=[["Technology / AI / cloud (Year 1)",60000,"Service infrastructure"],["Sales & marketing",120000,"Customer acquisition"],["Legal / compliance / audit",80000,"Offering, contracts, reporting and governance"],["Personnel (non-CEO)",300000,"Delivery and operating capacity"],["CEO compensation",120000,"Leadership compensation"],["Insurance",30000,"Risk management"],["Travel",40000,"Business development and delivery"],["Office / administration",30000,"Operating infrastructure"],["Other consultants",40000,"Specialist resources"],["Liquidity reserve",525000,"17.5% reserve midpoint"],["Working capital / contingency",null,"Balance available for multi-year deployment"]];
...
sc.getRange("A10:D10").values=[["Coupon sensitivity","2026-2029 cash interest","2029 ending cash impact vs. 9%","Interpretation"]]; header(sc,"A10:D10"); sc.getRange("A11:A13").values=[[0.08],[0.09],[0.10]];
...
const s=wb.worksheets.add("Investor Summary"); title(s,"SVT Nexus — Version 2 Investor Financial Summary","J");
...
s.getRange("G5:J12").values=[
 ["Note interest","ACT/365 from each funding date","No interest before receipt","Implemented"],["Quarterly cash interest","Separated from monthly accrual","Cash flow differs from expense","Implemented"],["Conversion","After forecast horizon","Principal remains debt through 2029","Disclosed"],["Client ramp","Base adds 3/3/2/2/2 Jan-May","Avoids same-day launch assumption","Implemented"],["Revenue cases","Downside / Base / Management","Avoids false precision","Implemented"],["Operating cash flow","Shown excluding financing","Surfaces self-sufficiency","Implemented"],["Use of proceeds","Reconciles to $3.0M","Explains capital deployment","Implemented"],["Professional review","Tax, ASC 606 and securities terms","Final documents remain subject to advisers","Required"]];
...
// Global polish
for(const sh of [a,cr,rev,op,note,inc,cf,u,sc,be,s]){
...
wb.comments.addThread({cell:a.getRange("B6")},"9% per annum per management clarification. Accrual begins on each tranche's actual funding date; quarterly cash payment is modeled separately.");
wb.comments.addThread({cell:a.getRange("B8")},"Conversion begins after 2029. No principal conversion is included within the 2026-2029 forecast horizon.");
...
for(const [name,range] of [["Investor Summary","A1:J20"],["Assumptions","A1:H21"],["Client Ramp","A1:M16"],["Revenue Model","A1:AL14"],["Operating Model","A1:AL13"],["Convertible Note","A1:AO22"],["Income Statement","A1:AL12"],["Cash Flow","A1:AL18"],["Use of Proceeds","A1:D16"],["Scenario Analysis","A1:F13"],["Break-Even","A1:C13"]]){

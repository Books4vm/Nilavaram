...
const OUT = "SVT_Nexus_Financial_Model_V4.xlsx";
...
const title = (s, text, endCol="H") => {
  ...
  s.mergeCells(`A2:${endCol}2`); s.getRange("A2").values=[["Version 4 | Model date: 21 Aug 2026 | Positive-income operating case — evidence and adviser review required"]];
...
// Assumptions
const a=wb.worksheets.add("Assumptions"); title(a,"SVT Nexus — Version 4 Assumptions","H");
...
const opex=[["CEO compensation",120000,"Management assumption"],["Other personnel / contractors",420000,"Staged 2027 deployment"],["Technology / AI / cloud",120000,"Staged 2027 deployment"],["Sales & marketing",300000,"Staged 2027 deployment"],["Legal / accounting / audit / tax / compliance",80000,"Planning estimate"],["Insurance",30000,"Planning estimate"],["Travel",40000,"Planning estimate"],["Office / administrative",30000,"Planning estimate"],["Other consultants / contractors",40000,"Planning estimate"]];
...
cr.getRange("A6:M6").values=[["Base — new clients",4,4,3,3,2,2,0,0,0,0,0,0]];
...
cr.getRange("A19:D21").values=[[2027,0,18,18],[2028,18,6,24],[2029,24,6,30]];
...
for(let i=0;i<months.length;i++){
 ...
 if(y===2027){
  rev.getRange(`${c}4`).formulas=[[`='Client Ramp'!${rampCol}$9*50%`]];
  rev.getRange(`${c}5`).formulas=[[`='Client Ramp'!${rampCol}$9*25%`]];
  rev.getRange(`${c}6`).formulas=[[`='Client Ramp'!${rampCol}$9*25%`]];
 } else {
  const total=y===2028?24:30;
...
cf.getRange("A4:A18").values=[["Beginning Cash"],["Customer Collections"],["Direct Project Costs"],["Operating Expenses"],["Operating Cash Flow (ex-financing, pre-tax)"],["Income Tax Paid"],["Operating Cash Flow (ex-financing, after-tax)"],["Convertible Note Proceeds"],["Cash Interest Paid (quarterly)"],["Financing Cash Flow"],["Net Cash Flow"],["Ending Cash"],["Reserved Cash (10% of funded face principal)"],["Available Operating Cash"],["Outstanding Convertible Note Principal"]];
for(let i=0;i<months.length;i++){
...
sc.getRange("A5:C7").values=[["Downside",6,"6-month delay; retainer-only"],["Base",18,"Jan-June staged ramp; differentiated pricing"],["Management",24,"Aspirational target case"]];
...
sf.getRange("B5:D9").values=[[360,180,180],[108,54,54],[36,18,18],[18,6,6],[18,24,30]];
...
const s=wb.worksheets.add("Investor Summary"); title(s,"SVT Nexus — Version 4 Investor Financial Summary","J");

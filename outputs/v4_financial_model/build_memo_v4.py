...
OUT="SVT_Nexus_2026_2029_Financial_Estimate_Memorandum_V4.docx"
...
hp=sec.header.paragraphs[0]; hp.text="SVT NEXUS  |  VERSION 4 FINANCIAL SUPPORT MEMORANDUM"; hp.alignment=WD_ALIGN_PARAGRAPH.RIGHT
...
p=d.add_paragraph(); p.alignment=WD_ALIGN_PARAGRAPH.CENTER; p.paragraph_format.space_before=Pt(14); r=p.add_run("VERSION 4  •  POSITIVE-INCOME OPERATING CASE"); r.font.size=Pt(11); r.font.bold=True; r.font.color.rgb=RGBColor.from_string(GRAY)
...
d.add_paragraph("Version 4 produces positive 2027 net income through an evidence-dependent 18-client ramp and staged operating deployment—not through a manual profit override. The model retains the 4% cash / 5% PIK structure, phased funding, bad debt, differentiated direct costs, full interest expense, and the Section 163(j) tax stress test. Ending clients increase to 18, 24, and 30 in 2027, 2028, and 2029.")
...
table(d,["Metric","2026","2027","2028","2029"],[
 ["Note proceeds","$150,000","$2,850,000","—","—"],["Gross revenue","—","$2,033,000","$3,686,760","$5,069,295"],["Operating income","$(42,500)","$394,940","$1,547,577","$2,484,868"],["Total interest expense","$(1,147)","$(167,042)","$(270,740)","$(270,000)"],["Net income","$(43,647)","$156,018","$944,859","$1,639,002"],["Ending cash","$106,990","$3,205,810","$4,301,081","$6,090,083"],["Note carrying amount","$150,637","$3,093,438","$3,243,849","$3,393,849"]
],[2.15,1.05,1.05,1.05,1.05])
d.add_paragraph("The 2027 positive result depends on converting 18 clients by June and staging part of the multi-year growth allocation beyond 2027. The result is compliant only to the extent these assumptions have a reasonable evidentiary basis and the offering package fully discloses execution risk.")
...
table(d,["Phase","Maximum","Release condition","Modeled schedule"],[
 ["Phase 1","$1,250,000","Initial runway and evidence-supported client build","December 2026–April 2027"],["Phase 2","$1,750,000","Prior-month annualized gross revenue of at least $1.0M","May–November 2027 if achieved"],["Maximum","$3,000,000","Both phases close","Subject to actual subscriptions and receipts"]
],[1.0,1.25,3.2,1.65])
...
table(d,["Term","Illustrative Version 4 assumption","Required action"],[
...
d.add_paragraph("Version 4 distinguishes the multi-year allocation from the 2027 expense budget. The 2027 model deploys $420,000 to non-CEO personnel, $300,000 to sales and enterprise marketing, and $120,000 to technology, with the remaining approved growth allocation staged into later periods as hiring, client, and product milestones are achieved. Total 2027 operating expense is $1.18 million.")
...
table(d,["Funnel metric","2027","2028","2029"],[
 ["Qualified leads","360","180","180"],["Demos / discovery","108","54","54"],["Paid or documented pilots","36","18","18"],["New clients","18","6","6"],["Ending clients","18","24","30"],["Lead-to-demo","30.0%","30.0%","30.0%"],["Demo-to-pilot","33.3%","33.3%","33.3%"],["Pilot-to-client","50.0%","33.3%","33.3%"],["Sales spend per new client","$16,667","$55,000","$60,500"]
],[2.8,1.2,1.2,1.2])
...
d.add_paragraph("Version 4 generates approximately $2.03 million of 2027 gross revenue, $394,940 of operating income, and $156,018 of net income after full cash/PIK interest and the modeled tax provision. Gross revenue then increases to approximately $3.69 million in 2028 and $5.07 million in 2029 as ending clients rise to 24 and 30.")
...
d.add_paragraph("At the post-July category mix, twelve equivalent clients generate $147,000 of gross monthly revenue and approximately a 77.2% contribution margin after bad debt and differentiated direct costs. With the staged 2027 monthly fixed operating budget of approximately $98,300, indicative gross-revenue break-even is approximately $127,400, or about 10.4 equivalent clients. The Base Case exceeds this level as the client ramp matures.")

d.add_heading('6. Tax Stress Test — Section 163(j)',1)
d.add_paragraph("Version 4 retains the simplified stress case limiting current deductible business interest to 30% of modeled adjusted taxable income when the toggle is active. The model produces no current tax provision in 2026, approximately $71,879 in 2027, $331,978 in 2028, and $575,866 in 2029 under that simplified convention.")
...
table(d,["Area","Version 4 disclosure","Required owner/action"],[
...
d.add_heading('Appendix A — Version 4 Proposed Term Register',1)
table(d,["Term","Version 4 assumption","Status"],[
...
d.core_properties.title="SVT Nexus 2026–2029 Financial Estimate and Offering Support Memorandum — Version 4"
